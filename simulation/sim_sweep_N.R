...
if(!LOAD){
  ...
  SNR_STRENGTH_VECTOR <- c(18, 25, 40, 60, 80) # 12
  K <- 10
  ...
  output_sim <- simulation_param_grid(param_name="n",               # The name of the parameter to vary
                                 # param_values=c(500,10000),
                                  param_values=c(1000, 2000, 5000, 10000, 20000),             # The values of the parameter to iterate over
  ...
  load(here("data/simulation_results", "simulation_output_N_sweep2_5.RData"))
...
# Generate the plots
sensitivity_plot <- plot_metrics_by_param(output, "n", "Sensitivity", log_scale = TRUE)  + theme(legend.position = "none")  # Remove legend
precision_plot <- plot_metrics_by_param(output, "n", "Precision", log_scale = TRUE)  + theme(legend.position = "none") 
...
ggsave(here("data/plots/combined_plot_n_5.png"), plot = combined_plot, width = 8, height = 8, dpi=600)


if (SAVE){
  sim_output <- list()
  sim_output$runs <- output_sim
  save(sim_output, file = here("data/simulation_results", "simulation_output_N_sweep2_5.RData"))
